...
This script takes the Watson 2020 catch data, a list of fish oil/fish meal (FOFM) species, and B/Bmsy estimates to calculate a score for FOFM species per OHI region. The FIS data prep will need to be completed prior to this data prep.  

## Updates from previous assessment

No methods updates. Just updated RAM data.
...
watson_old <- read_csv(file.path(here(), "globalprep/fis/v2020/int/watson_taxon_key_v2020.csv"))
...
watson_v5 <- read_csv(file.path(here(), "globalprep/fis/v2021/int/watson_taxon_key_v2021.csv")) %>%
  dplyr::select(1:3)
...
watson_combine <- watson_v5 %>%
  left_join(watson_old, by = c("Taxonkey", "TaxonName", "CommonName"))
...
watson_new_corr <- read_csv(file.path(here(), "globalprep/np/v2021/int/watson_new_corr.csv"))
...
write.csv(foragefish_list, here("globalprep/np/v2021/int/master_taxa_list_v5.csv"), row.names=FALSE)
...
foragefish_list <- read_csv(here("globalprep/np/v2021/int/master_taxa_list_v5.csv"))
...
write.csv(forage_fish_no_watson, file.path(here(), "globalprep/np/v2021/int/forage_fish_not_in_watson.csv"), row.names = FALSE)
...
write.csv(catch_fishfeed, file.path(here(), "globalprep/np/v2021/int/watson_catch_forage_fish.csv"), row.names = FALSE)
```
...
```{r}
catch_fishfeed <- read_csv(file.path(here(),"globalprep/np/v2021/int/watson_catch_forage_fish.csv"))
...
plot(score_data$score, old$score) # some changes
