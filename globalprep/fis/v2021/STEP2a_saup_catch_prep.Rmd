...
# test <- fao_eez_prod %>%
#   filter(str_detect(area_name, "Pitcairn"))
# 
# test2 <- test %>%
#    group_by(year) %>%
#   summarise(sum = sum(tonnes_fix))
...
## gibraltar, monaco, oecussi ambeno tricky - will gapfill at the end.. assign them the scores of their admin regions (Spain, France, East Timor)
## bouvet is included? Not enough catch? Protected area anyways; uninhabited?
...
# test <- read.csv(file.path(fis_path, "ohi_rgn_eez_prod.csv")) %>%
#   filter(rgn_id == 146)
# 
# test2 <- test %>%
#   group_by(year) %>%
...
# test <- read.csv(file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn.csv')) %>%
#   filter(rgn_id == 146)
# 
# test2 <- test %>%
#   group_by(year) %>%
...
# 
# test <- read.csv(file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn_taxa.csv')) %>%
#   filter(rgn_id == 146)
# 
# test2 <- test %>%
#   group_by(year) %>%
...
Take a look at catch data with missing ohi and fao regions in **stock_catch_by_rgn_taxa**. These have taxon key matches, but no ohi or fao regions assigned to them. There are none for v2021!
```{r, eval=FALSE}

df <- read_csv(file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn_taxa.csv'))

# 0 NAs for OHI regions
df_na <- df %>% 
  filter(is.na(rgn_id))
nrow(df_na)

# 0 catch data without fao regions assigned - v2021
df_na <- df %>% 
