...
## FAO capture production data - all columns being parsed as characters and producing error in one column, but not sure which? (read.csv might help avoid this error?) # ERIKA: can i delete this?
...
sp2grp <- read_csv(here('globalprep', 'prs_targetedharvest', paste0('v', version_year, '_e'), 'raw', 'species2group.csv')) %>% # ERIKA: can remove the _e in final
...
fao_clean <- fis_fao_raw %>% 
  ...
  dplyr::select(-c("Unit (Name)", "Unit")) %>% # ERIKA: added removing Unit also
...
fao_clean <- fao_clean %>%
  ...
  # dplyr::mutate(year = gsub("X", "", year)) %>% # ERIKA: not needed now? i commented out
    fao_clean_data_new() 
...
#     Eastern Pacific green turtle # ERIKA: why is this?
...
wide <- target_spp %>%
  ...
  # dplyr::select(-`NA`) %>% # ERIKA: Column `NA` doesn't exist. so i commented out
...
write_csv(
  target_harvest, 
  here('globalprep', 'prs_targetedharvest', paste0('v', version_year, '_e'), 'output', 'fao_targeted.csv')
) # ERIKA: remove _e for final
...
write_csv(
  target_harvest_gf,
  here('globalprep', 'prs_targetedharvest', paste0('v', version_year, '_e'), 'output', 'fao_targeted_gf.csv')
) # ERIKA: remove _e for final
```
...
new <- readr::read_csv(here("globalprep", "prs_targetedharvest", paste0('v', version_year, '_e'), "output", "fao_targeted.csv")) %>% # ERIKA: remove _e for final
