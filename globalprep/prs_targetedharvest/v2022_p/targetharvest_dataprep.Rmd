...
fis_fao_raw <-  read_csv(file.path(dir_M, 'git-annex/globalprep/_raw_data/FAO_capture/d2022/Global_capture_production_Quantity_1950-2020.csv'))
...
# pivot wider spread to expand years
wide = target_spp %>%
  tidyr::pivot_wider(names_from = year, values_from = value) %>%
  dplyr::left_join(sp2grp, by = 'species'); head(wide)


# pivot longer by target
long = wide %>%
  dplyr::select(-area) %>%
  tidyr::pivot_longer(cols = !c(country, species, target), names_to = "year",
...
target_harvest <- m_sum_rgn %>%
  dplyr::mutate(quant_95 = quantile(value[year %in% 2011:(current_year - 2)], 0.95, na.rm = TRUE)) %>%
...
target_harvest <- rgns %>%
  ...
  dplyr::mutate(pressure_score = tidyr::replace_na(pressure_score, 0)) %>%
...
write_csv(target_harvest, 
          here('globalprep', 'prs_targetedharvest', paste0('v', current_year, '_p'), 'output', 'fao_targeted.csv'))
...
write_csv(target_harvest_gf,
          here('globalprep', 'prs_targetedharvest', paste0('v', current_year, '_p'), 'output', 'fao_targeted_gf.csv'))
...
new <- read_csv(here('globalprep', 'prs_targetedharvest', paste0('v', current_year, '_p'),
                     'output', 'fao_targeted.csv')) %>% 
  filter(year == (current_year - 3))
# pull just 2019 data from target_harvest df, since its the most recent year in common

old <- read_csv(here('globalprep', 'prs_targetedharvest', paste0('v', (current_year - 1)), 'output', 'fao_targeted.csv')) %>%
  dplyr::filter(year == (current_year - 3)) %>%
...
indonesia <- long %>% 
  dplyr::group_by(country, target, year) %>%
  dplyr::summarize(value = sum(value)) %>% 
  dplyr::filter(country == 'Indonesia', year >= 2000) 
```
