...
## 830 is considered region by UN, we call it country and split it to islands later
regions <- c(
...
clean_data <- api_get_indicator_data$data %>% 
  ...
  dplyr::mutate(
    ...
    region_type = case_when(
      ...
      TRUE ~ "country")) %>%
   # dplyr::select(geo_area_code, geo_area_name, time_detail, value) %>% 
  dplyr::filter(!(geo_area_code %in% c(344, 446))) %>% # filter out hongkong/macao, they are NA anyways
  dplyr::left_join(region_info, by = c("geo_area_code" = "m49_code")) %>% 
...
## v2021 - v2022 matches as well
## removed: Aland (not OHI), Bonaire Sint Saba (fixed above), Channel Islands (fixed in new data), Eswatini (not OHI), French southern territories (fixed above), Isle of man (not OHI), North Macedonia (land locked), Saint Barthelemy (not OHI), Palestine (not OHI), USMOI (fixed above) - perfect! 
...
# v2022 Micronesia  and Micronesia (Federated States of) disappeared too...
# # A tibble: 7 × 1
#   geo_area_name               
#   <chr>                       
# 1 China                       
...
## filter for these regions in country_df and prep so they match rgns_data

deleted_rgns <- country_df %>%
...
kiribati_split <- deleted_rgns %>%
  dplyr::filter(geo_area_name %in% c("Kiribati")) %>%
  dplyr::mutate(geo_area_name = ifelse(
    geo_area_name == "Kiribati", 
...
all_rgns_data_df <- rbind(rgns_data, all_deleted_fixes) %>%
...
remaining_rgns <- data.frame(
  geo_area_name = c(
    "Andaman and Nicobar", "Ascension", "Azores", "Canary Islands", "Madeira",
    "Oecussi Ambeno", "Prince Edward Islands", "Tristan da Cunha"), 
