...
# sort(unique(fis_bbmsy$rgn_name))
# 
...
length(unique(catch_AO$rgn_id)) # there are only 210 regions here... we want there to be 220 regions. Which regions are missing? 
...
setdiff(rgns_eez$rgn_id, test$rgn_id)
 ...
 # [1] "Belgium"           "Latvia"            "Estonia"           "Kerguelen Islands" "Faeroe Islands"    "Iceland"          
 # [7] "Finland"           "Poland"            "Lithuania"         "Antarctica"        "Sweden"   


## So we need to gapfill these regions somehow... do they have b/bmsy data?

missing <- c("Belgium", "Latvia", "Estonia", "Kerguelen Islands", "Faeroe Islands", "Iceland", "Finland",           "Poland", "Lithuania", "Sweden")

missing_id <- c(59, 69, 70, 93, 141, 143, 174, 178, 189, 222)

test <- fis_bbmsy %>%
  left_join(rgns_eez) %>%
  filter(rgn_name %in% missing)

unique(test$rgn_name)
...
b <- b %>%
  ...
  data_fis_final <- rbind(data_fis, gapfill_missing)
  
  unique(data_fis_final$rgn_id) # 220 regions, perfect
