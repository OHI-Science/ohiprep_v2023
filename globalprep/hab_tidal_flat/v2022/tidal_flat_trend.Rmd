...
tidal_flat_trend_2013 <- tidal_flat_extent %>%
  ...
  dplyr::mutate(trend = estimate * 5, year = 2013) %>% 
  dplyr::select(rgn_id, rgn_name, year, habitat, trend) %>% 
  dplyr::ungroup()
```
...
tidal_flat_trend_2016 <- tidal_flat_extent %>%
  ...
  tidyr::unnest(cols = c(model)) %>% 
  dplyr::filter(term == "year") %>% 
  dplyr::mutate(trend = estimate * 5, year = 2016) %>% 
  dplyr::select(rgn_id, rgn_name, year, habitat, trend) %>% 
  dplyr::ungroup()
```
...
```{r eval = FALSE}
tidal_flat_trend <- rbind(tidal_flat_trend_2013, tidal_flat_trend_2016) %>% 
