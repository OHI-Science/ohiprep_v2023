...
tidal_flat_condition <- tidal_flat_extent %>% 
  ...
  dplyr::mutate(condition = `2016` / `1986`) %>% 
  tidyr::replace_na(list(condition = 0)) %>% 
  dplyr::select(rgn_id, rgn_name, habitat, condition) %>% 
  dplyr::mutate(condition = case_when(condition > 1 ~ 1, T ~ condition)) 
```

## Write the results

```{r eval = FALSE}
readr::write_csv(tidal_flat_condition, here::here(dir_here, "output", "tidal_flat_condition.csv"))
```
