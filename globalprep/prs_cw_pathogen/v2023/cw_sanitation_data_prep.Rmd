...
Updates on our end: 
1. Updated data download through the SDMX REST API to programmatically access data. We are looking at the wash_household indicator. 


#old delete at the end:
...
version_year <- paste0("v", 2023) #update this 
data_start <- 2000
data_end <- 2022
...
link to api: https://sdmx.data.unicef.org/webservice/data.html

query information: https://data.unicef.org/sdmx-api-documentation/#understanding
...
Some countries are missing a value for the at least basic category indicator (ws_ppl_s_alb). This can be calculated by adding the proportion of population using limited sanitation services, the proportion of population using unimproved sanitation services, and the proportion of the population using open defecation, and subtract from 100 to get the value if it doesn't exist. 
...
sanitation_clean <- sanitation_wider %>%
  ...
  mutate(alb = 100 - not_basic) %>% #calculate at least basic from the other 3 indicatord
...
#add in channel islands because not recognized by the function
sanitation_clean <- sanitation_clean %>% mutate(country = case_when(country_code == "CHI" ~ "Channel Islands",
                                                                TRUE ~ country))
...
sanitation_clean <- sanitation_clean %>% 
  ...
  mutate(population = case_when(str_detect(country, "Martinique") ~368000,
         ...
         is.na(population) ~ 1,
         TRUE ~ population)) #add static value for Guadeloupe and Martinique since these are not reported


#sani_test <- sani %>% left_join(sanitation_clean, by = c("country_id" = "country_code", "year"= "year"))
```
...
sanitation_clean <- sanitation_clean %>%
  ...
  mutate(country = case_when(str_detect(country,"Cura\\Sao") ~"Curacao",
                            str_detect(country,"R\\Sunion") ~ "Reunion",
                            str_detect(country, "C\\Ste d\\SIvoire") ~ "Ivory Coast",
                            str_detect(country,"Hong Kong") ~ "Hong Kong",
                            str_detect(country,"Macao") ~ "Macao",
                           str_detect(country, "Saint Martin") ~ "Northern Saint-Martin",
                           str_detect(country, "Helena") ~"Saint Helena",
                           str_detect(country, "São Tomé and Príncipe")~ "Sao Tome and Principe",
                           str_detect(country, "St. Vincent and Grenadines") ~ "Saint Vincent and the Grenadines",
                           str_detect(country, "U.S. Virgin Islands")~ "Virgin Islands (U.S.)",
                           str_detect(country, "Congo - Kinshasa") ~ "Democratic Republic of the Congo",
                           str_detect(country, "Congo - Brazzaville") ~"Republique du Congo",
                           str_detect(country, "Myanmar") ~ "Myanmar",
                          TRUE ~ country
                           ))
...
## Each region has 23 years of data in this case. If there are regions with more than 23 values is because in this database they are reported in a higher resolution than the OHI global assessment (eg: China, Hong Kong and Macao are just one region for the OHI global). For these regions a weighted average will be calculated as its final score.
...
sani_final <- rgn_sani %>% 
  ...
  dplyr::summarise(basic_sani_prop= weighted.mean(basic_prop, population, na.rm = TRUE)) %>% 
...
#this will need to be updated with the last version year 
sani_complete_old <- read_csv(here("globalprep/prs_cw_pathogen/v2021/intermediate/sani_complete.csv")) %>% 
...
unsani_prs_old <- read_csv(here("globalprep/prs_cw_pathogen/v2021/output/po_pathogen_popdensity25mi.csv")) %>%
...
compare <- read_csv(here("globalprep/prs_cw_pathogen/v2021/output/po_pathogen_popdensity25mi_trend.csv")) %>%
