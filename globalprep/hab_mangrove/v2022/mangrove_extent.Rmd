...
# the values in this raster are rgn_ids for countries and their eezs, but it includes high seas and Antarctica which have really large numbers as ids
land_ocean[land_ocean > 250] <- NA
# plot(land_ocean)
```

Read in GMW data and explore
```{r, eval = FALSE}
files <- list.files("/home/shares/ohi/git-annex/globalprep/_raw_data/wcmc_mangrove/2017-2020/")
...
for (i in seq_along(files_2)) {
  ...
  if(!file.exists(rasterize_file)) {
    ...
    gmw_shp <- terra::vect(file.path(dir_wcmc, "2017-2020", gmw_file))
...
```{r, eval = FALSE}
gmw_ohi_2016 <- read.csv(here(paste("globalprep/hab_mangrove/v", version_year, "/int/habitat_extent_mangrove_2016.csv")))
