...
ref_areas <- fao_gmw %>% 
  ...
  mutate(ref_year = case_when(!is.na(km2_1980) ~ 1980,
                              is.na(km2_1980) & !is.na(km2_ref) ~ 1996)) %>% 
  select(rgn_id, km2_ref, ref_year)
...
write.csv(habitat_health_mangrove_updated, file.path(dir_hab_mangrove, "data/habitat_health_mangrove_updated.csv"))
```
...
Based on this comparison plot, health scores have changed a lot across the board. This isn't entirely unexpected as current extents have been updated from 2005 to 2020, and the GMW data are likely much more accurate than the FAO data.
