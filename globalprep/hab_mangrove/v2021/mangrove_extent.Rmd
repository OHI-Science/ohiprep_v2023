...
This script generates the extent of mangrove for each OHI region for the latest year of data (year == 2016). We do this using polygon data. We extract the area of each polygon per each ohi region.
...
sum(old_extent$km2) # 118507.7

## Our new extent is much larger than our old extent 

old_extent_fix <- old_extent %>%
...
compare_habitat_extent <- gmw_ohi_2016 %>%
  filter(rgn_id <= 250) %>%
  left_join(old_extent_fix, by = "rgn_id") %>%
  mutate(km2.y = ifelse(
...
sum(compare_habitat_extent$km2.x) - sum(compare_habitat_extent$km2.y) # 18182.27 km2 increase 
