...
# suppress the extra files (.tif.aux.json) that are produced by `terra` in addition to the .tif output when using writeRaster()
#rgdal::setCPLConfigOption("GDAL_PAM_ENABLED", "FALSE")
# setGDALconfig("GDAL_PAM_ENABLED", "FALSE")

# stack overflow that mentioned the first line works if install the dev version of terra package:
...
#p4s_wgs84 <- "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0" # proj4string, need 
# might need to update crs string because switching from raster pkg to terra pkg and an error is resulting with terra::project() downstream

cols <- rev(colorRampPalette(brewer.pal(9, "Spectral"))(255)) # rainbow color scheme for maps

# new workflow this year, integrating the terra package instead of raster package
...
# not sure if the following line will change how the CRS is assigned
#mollCRS <- terra::crs("+proj=moll") # do not use proj 84 format

# WKT as HTML:
# mollCRS <- "PROJCS['World_Mollweide',
#     GEOGCS['GCS_WGS_1984',
#         DATUM['WGS_1984',
...
# WKT OGC
# mollCRS <- "PROJCS['World_Mollweide',GEOGCS['GCS_WGS_1984',DATUM['WGS_1984',SPHEROID['WGS_1984',6378137,298.257223563]],PRIMEM['Greenwich',0],UNIT['Degree',0.017453292519943295]],PROJECTION['Mollweide'],PARAMETER['False_Easting',0],PARAMETER['False_Northing',0],PARAMETER['Central_Meridian',0],UNIT['Meter',1],AUTHORITY['EPSG','54009']]"

# WKT ESRI
#mollCRS <- "PROJCS['World_Mollweide',GEOGCS['GCS_WGS_1984',DATUM['D_WGS_1984',SPHEROID['WGS_1984',6378137,298.257223563]],PRIMEM['Greenwich',0],UNIT['Degree',0.017453292519943295]],PROJECTION['Mollweide'],PARAMETER['False_Easting',0],PARAMETER['False_Northing',0],PARAMETER['Central_Meridian',0],UNIT['Meter',1]]"

#mollCRS <- terra::crs("+proj=moll +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs")
#mollCRS <- terra::crs("+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs")
#mollCRS <- "+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
#mollCRS <- "+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
#mollCRS <- "+proj=moll +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs"
#mollCRS <- terra::crs("ESRI:54009")
#mollCRS <- "ESRI:54009"
#mollCRS <- terra::crs("SR-ORG:7099")
...
ub <- sprintf("ftp://%s@%s", userpasswd, substr(url, 7, 87)) # here, substr extracts the url string characters 7-87, which encompasses everything in the object url after "ftp://"
```
...
# juliet's notes when trying to change code to pull files:

# ub = path to folder from which we retreive data, which is the server path with userpasswd and folder that contains all files:
# "ftp://userpasswd@ftp-access.aviso.altimetry.fr/climatology/global/delayed-time/monthly_mean/msla_h"

# u = path to data file (which includes path to folder (ub)
# "ftp://userpasswd@ftp-access.aviso.altimetry.fr/climatology/global/delayed-time/monthly_mean/msla_h" +
# /dt_global_allsat_msla_h_y2020_m +
# mo (ranges from 01 -12) +
# .nc
```
...
# Note: file paths within object nc_files is inconsistent across years due to how data is stored by the source:
# files for years 1993 - 2015 are named as the following:
# "/home/shares/ohi/git-annex/globalprep/_raw_data/AVISO_slr/d2016/msla_monthly_mean/dt_global_allsat_msla_h_y1993_m01.nc"
# files for years 2016 - 2020 are named as the following:
# "/home/shares/ohi/git-annex/globalprep/_raw_data/AVISO_slr/d2017/dt_global_allsat_msla_h_y2016_m12.nc"
...
# removed axes = F arg
#plot(raster::raster(nc_files[3]), col = cols, 
#     main = paste("Year", substr(nc_files[3], 90, 93), "Month", substr(nc_files[3], 96, 97)))

# compare to rast() in the terra package
# removed axes = F arg
...
```{r}
# the rasters look the same, but let's compare the two rasters as dataframes and compare their values

# set fill scale limits based on raster values, from 0 to at least 1 ????
#ocean_terra_lim <- terra::minmax(ocean_terra)

# define the rasters as objects to compare between packages
raster_y2020_m03 <- raster::raster(nc_files[3])
terra_y2020_m03 <- terra::rast(nc_files[3])

# convert to df's
raster_y2020_m03_df <- as.data.frame(raster_y2020_m03, xy = TRUE, na.rm = FALSE)
terra_y2020_m03_df <- as.data.frame(terra_y2020_m03, xy = TRUE, na.rm = FALSE)
# recode NaN to NA value
terra_y2020_m03_df$sla[terra_y2020_m03_df$sla == "NaN"] <- NA
#unique(raster_y2020_m03_df$Averaged.Sea.Level.Anomalies.2020.03)

true_false_df <- as.data.frame(raster_y2020_m03_df == terra_y2020_m03_df) # all values are NA or TRUE, no FALSE values in any column
#unique(true_false_df$Averaged.Sea.Level.Anomalies.2020.03)
...
```{r, eval = F}
# THIS IS THE OLD WAY, USING RASTER::RASTER() INSTEAD OF TERRA::RAST()
...
#    raster::projection(r) <- "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
# 
#    ## write raster to int folder in prs_slr
#    fp <- sprintf("%s/int/msla_monthly/msla_monthly_%s.tif", dir_prs_slr, m_yr)
#    raster::writeRaster(r, filename = fp, overwrite = TRUE)
#  }
# this chunk worked, but now I have to get the next chunk to work since we are transitioning from raster to terra
# check on the many warnings that result from this chunk (check if they result from the terra approach too)
```
...
# foreach(file = nc_files) %dopar% {
#   
#   m_yr <- substr(file, nchar(file)-10, nchar(file)-3) # m_yr value example: "2015_m12"
# 
#   ## read in month raster
#   r <- terra::rast(file) %>%
#     rotate()
...
#   
#   # new code to solve error in writing the files" Error in x@ptr$nrow() : external pointer is not valid
#   # wrap() creates a packed obj that enable them to be saved to disk or passed to a connection that serializes (as we do in this loop)
#   #r_packed <- terra::wrap(r)
#   #r_packed <- pack(r) # this function on its own returned the error Error in { : task 1 failed - "`x` must be a vector, not a <SpatRaster> object."
#  # r_unpacked <- unpack(r_packed) 
#   # set values in place 
#   #set.values(r)
#   #values(r) <- values(r)
#   ## write raster to int folder in prs_slr
...
```{r}
tic()
# do not run in parallel because it is not compatible with terra (package might be updated in the future to accomodate parallelization)
# rotates each monthly file, sets the long/lat projection, and keeps only coastal cells - saved to GitHub
# takes ~201-370 seconds to run
...
#mollCRS <- CRS("+proj=moll") # mollweide projection
# not sure if the following line will change how the CRS is assigned
#mollCRS <- terra::crs("ESRI:54009") # do not use proj 84 format
# mollCRS <- "PROJCS['World_Mollweide',
#     GEOGCS['GCS_WGS_1984',
#         DATUM['WGS_1984',
#             SPHEROID['WGS_1984',6378137,298.257223563]],
...
# 
# 
#   rast_data <- terra::rast(file) %>%
#     #projectRaster(crs = mollCRS, over = TRUE) %>%
#     terra::project(mollCRS) %>%  # terra::project does not have an argument 'over' that was present in raster::projectRaster() that was set to TRUE to avoid mapping the same area twice, which is desirable for globla data, but I do not think this is an issue
...
# this threw the same error as the last toime i tried to use foreach with terra: Error in x@ptr$nrow() : external pointer is not valid
```

```{r}
tic()
# unparallelize because working with terra crs and foreach does not work
