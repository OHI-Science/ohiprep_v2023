...
#mollCRS_terra <- terra::crs("+proj=moll")
...
```{r}
# check that raster() and rast() do the same thing
ocean_raster
...
# compare ocean_raster to ocean_terra
#raster_y2020_m03_df <- as.data.frame(raster_y2020_m03, xy = TRUE)
# explore vaues
#colnames(raster_y2020_m03_df)
#unique(raster_y2020_m03_df$Averaged.Sea.Level.Anomalies.2020.03)
# this col has a lot of NA's but does have a large range of real numeric values
# determine how many NA's
#sum(is.na(raster_y2020_m03_df$Averaged.Sea.Level.Anomalies.2020.03))

#terra_y2020_m03_df <- as.data.frame(terra_y2020_m03, xy = TRUE)
#sum(is.na(terra_y2020_m03_df$sla))

# set fill scale limits based on raster values, from 0 to at least 1
#ocean_terra_lim <- terra::minmax(ocean_terra)

# maybe convert them to a matrix before a df
# ocean_raster_matrix <- as.matrix(ocean_raster)
# ocean_terra_matrix <- as.matrix(ocean_terra)
...
#unique(true_false_df$y)

# determine if the values need to be set for both raster and terra before converting to a df
#raster::values(raster_y2020_m03) <- 1:ncell(raster_y2020_m03)
#terra::values(terra_y2020_m03) <- 1:ncell(terra_y2020_m03)

#raster_y2020_m03_mtx <- as.matrix(raster_y2020_m03)
#terra_y2020_m03_mtx <- as.matrix(terra_y2020_m03)
...
```{r}
tic()
# do not run in parallel because it is not compatible with terra 
# rotates each monthly file, sets the long/lat projection, and keeps only coastal cells - saved to GitHub
# takes 203 seconds to run

for (i in seq_along(nc_files)){
  
  m_yr <- substr(nc_files[i], nchar(nc_files[i])-10, nchar(nc_files[i])-3)
...
## unparalleize the function in order to convert to terra
