...
- The data source's file path changed slightly, so this script was updated for the file paths in `sprintf()` and `sub-str()`
...
# }
```
2020 data is stored in the source as unzipped .nc files, so the above code chunk was not necessary. This is likely the case for future years.
...
```{r}
# define the raster as an obj to compare between packages
