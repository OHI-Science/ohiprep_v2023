...
```{r}
tic()
# do not run in parallel because it is not compatible with terra::project() (package might be updated in the future to accomodate parallelization)
...
# this threw the same error as the first chunk with `terra::project()` i tried to use foreach with terra: Error in x@ptr$nrow() : external pointer is not valid
```
...
#     
#     terra::rast(file) %>%
#     terra::app(fun = function(x){ifelse(x < 0, 0, x)}) %>% # set all negative values to 0
#     terra::app(fun = function(x){ifelse(x > ref, 1, x/ref)}, # set equal to one if greater than ref, otherwise scale
#          filename = sprintf("%s/output/slr_%s.tif", dir_prs_slr, yr), overwrite = TRUE) 
#   }
# }

# unparallelize because foreach() did not work with terra
...
```{r}
# check raster values
test_rast <- terra::rast("/home/shares/ohi/git-annex/globalprep/prs_slr/v2022/output/slr_2020.tif")
#plot(test_rast)
#test_rast_df <- as.data.frame(test_rast, xy = TRUE, na.rm = TRUE)
values(test_rast)
terra::unique(test_rast)
```
...
#zones <- crop(zones, stack_slr[1:28])
#cropped <- terra::rasterize(stack_slr, zones, "test")

## extract data for each region
## fyi takes awhile... about 2 hours for v2019.... 
#terra::extent(stack_slr) <- terra::extent(zones)
regions_stats <- zonal(stack_slr, zones, fun = "mean", na.rm = TRUE, progress = "text") %>% data.frame() # removed progress = "text" arg 

raster <- terra::rast(rasts[1])
#ext(raster)
ext(zones)

# iterate through layers of stack_slr to compute zonal statistics one by one
for(i in seq_along(rasts)){
  raster <- terra::rast(rasts[i])
  region_stats <- terra::zonal(raster, zones, fun = "mean", na.rm = TRUE) %>% data.frame()


 # for (j in seq_along(rast_layer)){
 #   terra::zonal(rast_layer[i], zones, fun = "mean", na.rm = TRUE, as.raster = TRUE)
 # }
}
