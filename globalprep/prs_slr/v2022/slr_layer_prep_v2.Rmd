...
# give the raster values before converting to a df (i THINK this is necessary for terra pkg)
terra::values(ocean_terra) <- 1:ncell(ocean_terra)
```
...
# ub = "ftp://userpasswd@ftp-access.aviso.altimetry.fr/climatology/global/delayed-time/monthly_mean/msla_h"

## download is quite fast, takes less that 3 minutes for 12 files
for (mo in months){ 
   #mo="01" # for testing
  
  ## retrieve the data (compressed, gzipped files prior to 2020 data, and just .cv files starting with 2020 data)
  ...
  u_filename <- file.path(dir_anx_aviso, paste0("/d", scen_year, "/test/"), substr(u, 121, 153)) #updated script in 2020 to include full filename
...
# check total number of characters in u to count backwards instead of counting forwards 
#nchar("ftp://clawson%40nceas.ucsb.edu:pMfKED@ftp-access.aviso.altimetry.fr/climatology/global/delayed-time/monthly_mean/dt_global_allsat_msla_h_y2020_m01.nc.gz") # 152

#nchar("ftp://clawson%40nceas.ucsb.edu:pMfKED@ftp-access.aviso.altimetry.fr/climatology/global/delayed-time/monthly_mean/msla_h") # 119

# substr(u, 115, 155)) = _y2020_m01.nc.gz (for first month) meaning this is the subset of the string to be replaced or this is the subset of the string that becomes the filename in the folder? It is hard to tell because I am unsure if last year's fellow extracted the files via cyberduck or with this code, which might result in diff file names 

# file.path:
# "/home/shares/ohi/git-annex/globalprep/_raw_data/AVISO_slr"
# "/d2022/test/" +
# _y2020_m01.nc (for first month)

# to pick up next shift: re-running userpasswrd to define in console right before running loop got rid of the error that said access denied, the error that resulted is  Error: Path exists and overwrite is FALSE, not sure how to fix that issue, might have to move current files (that were pulled from cyberduck) to other folder (within d2022 should be fine) so the file names are not in the directory the loop tries to populate
# i am suspicious that the way i am running userpasswd in the console is wrong? I run it and then hit enter again and I think it works...
# note: I am not sure why this code ends the files in gz, they do not appear to end in gz on the website for any year, maybe when we pull the files in with this code it automatically converts them to gz, which results in the need to unzip in the next chunk?
```

Unzip the files you've just downloaded: 
```{r, eval = FALSE}
# since the source changed the file types this year from nc.gz to just nc, we do not have to unzip them starting with 2022
...
#ocean_raster_df <- as.data.frame(ocean_raster)
#ocean_terra_df <- as.data.frame(ocean_terra)
