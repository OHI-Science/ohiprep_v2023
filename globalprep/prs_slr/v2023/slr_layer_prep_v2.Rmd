...
DELETE - The data source's file path (url) changed slightly, so this script's file paths were updated within `sprintf()` and `sub-str()`
DELETE - The data source's data storage format changed, so not necessary anymore to unzip the files after import.
DELETE - Converted `raster` functions to `terra`.
DELETE - Due to incompatibility with `terra::project()` and parallelization, 2 loops were converted from using `foreach` to simple for loops.
...
#### ERIKA
required_packages <- c("httr", "R.utils", "raster", "tidyverse", "sf", "RColorBrewer", "googleVis", "maps", "parallel", "foreach", "doParallel", "fasterize", "rasterVis", "terra", "tictoc", "here") # create list of required packages (add any new packages here)
