...
- Had to change `terra` code back to `raster` for one chunk (making the histograms) to get it to work
...
# create v20?? folder in supplementary_information
...
r_new <- raster::raster(sprintf("%s/output/slr_%s.tif", dir_prs_slr, scen_year - 2))
# get current scenario year most recent year of data IN COMMON (ie. will likely be one more year back)
r_new_old <- raster::raster(sprintf("%s/output/slr_%s.tif", dir_prs_slr, scen_year - 3))
# get previous scenario year most recent year of data IN COMMON (ie. will likely be one more year back)
r_old <- raster::raster(sprintf("%s/git-annex/globalprep/prs_slr/v%s/output/slr_%s.tif", dir_M, scen_year - 1, scen_year - 3))
...
rasterVis::histogram(r_new, main = sprintf("Sea Level Pressure %s current version (v%s) data", scen_year - 2, scen_year)) # v2023 - plots v2023 2021 data
rasterVis::histogram(r_new_old, main = sprintf("Sea Level Pressure %s current version (v%s) data", scen_year - 3, scen_year)) # v2023 - plots v2023 2020 data
rasterVis::histogram(r_old, main = sprintf("Sea Level Pressure %s previous version (v%s) data", scen_year - 3, scen_year - 1)) # v2023 - plots v2022 2020 data
```
