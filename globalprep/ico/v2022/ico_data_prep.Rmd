...
readr::write_csv(ico_list, here::here(dir_here, 'int', 'ico_list_prepped.csv')) # 270 in v2021
```
...
# 
# write_csv(ico_spp_countries, file.path(dir_here, 'int/ico_spp_countries.csv')) # save this as an intermediate data file since it takes so long to create the df
# 
...
for(j in 1:n_chunks){
  ...
  chunk_file <- file.path(
    dir_here, 'tmp', sprintf('spp_countries_chunk_%s.csv', n_chunks[j]))
...
fun_fun <- function(x) {readr::read_csv(x) %>% dplyr::mutate(code = as.character(code))}
...
country_split_1 <- data.frame(
  country = "Bonaire, Sint Eustatius and Saba", 
  region = c('Bonaire', 'Saba', 'Sint Eustatius')
)

country_split_2 <- data.frame(
  country = "French Southern Territories", 
  region = c('Glorioso Islands', 'Juan de Nova Island', 
             'Bassas da India', 'Ile Europa', 'Ile Tromelin',
             'Crozet Islands', 'Amsterdam Island and Saint Paul Island',
             'Kerguelen Islands')
)

country_split_3 <- data.frame(
  country = "Saint Helena, Ascension and Tristan da Cunha", 
  region = c('Ascension', 'Saint Helena', 'Tristan da Cunha')
)

country_split_4 <- data.frame(
  country = "United States Minor Outlying Islands",
  region = c('Wake Island', 'Jarvis Island', 'Palmyra Atoll', 'Howland Island and Baker Island', 'Johnston Atoll'))
...
write_csv(ico_spp_rgn_raw, here::here(dir_here, 'raw', 'ico_spp_rgn_raw.csv'))
```
...
write_csv(ico_spp_rgn_prepped, here::here(goal, version_year, 'int', 'ico_spp_rgn_prepped.csv'))
...
ico_spp_rgn_prepped <- ico_spp_rgn_prepped %>% 
  dplyr::select(iucn_sid, sciname, comname, presence, origin, distribution_code, cat, ico_gl, rgn_id, rgn_name, subpop, ico_rgn_id)
...
for(j in 1:n_chunks){
  ...
  chunk_file <- file.path(dir_here, 'tmp', 
...
spp_past_assess_chunk_files <- list.files(here::here(dir_here, 'tmp'), 
...
v2019_grouped <- here::here(goal, 'v2019','raw', 'ico_assessments_raw.csv') %>% 
  read_csv() %>% 
  group_by(iucn_sid) %>% 
  count() %>% 
  rename(v2019 = "n")
...
v2019 <- here::here(goal, "v2019", "raw", "ico_assessments_raw.csv") %>% 
  read_csv() %>% 
...
write_csv(ico_assess_raw, file.path(dir_here, 'raw/ico_assessments_raw.csv'))
```
...
write_csv(ico_assess, here::here(dir_here, 'int', 'ico_assess_clean.csv'))
...
ico_assess <- here::here(dir_here, 'int', 'ico_assess_clean.csv') %>% read_csv()
ico_list <- here::here(dir_here, 'int', 'ico_list_prepped.csv') %>% read_csv()
...
v2020_cat <- here::here(goal, previous_scen, 'int', 'ico_spp_cat.csv') %>% 
  read_csv() #has 715-10118-1540 NAs

write_csv(ico_spp_cat, here::here(dir_here, 'int', 'ico_spp_cat.csv'))
...
ico_spp_rgn <- here::here(goal, version_year, 'int', 'ico_spp_rgn_prepped.csv') %>% 
  read_csv(col_types = "ddcccccdcccldcc") %>% 
...
write_csv(ico_spp_rgn_cat, here::here(dir_here, 'int', 'ico_spp_rgn_cat.csv'))
...
ico_spp_rgn_cat <- here::here(dir_here, 'int', 'ico_spp_rgn_cat.csv') %>% 
  read_csv(col_types = "dcdcdddccldc")
...
write_csv(ico_sum, here::here(dir_here, 'summary', 'ico_summary.csv'))
...
write_csv(ico_status_raw1, here::here(dir_here, 'output', 'ico_spp_iucn_status.csv'))
write_csv(ico_status_calc, here::here(dir_here, 'output', 'ico_status_calc.csv'))
write_csv(ico_trend,       here::here(dir_here, 'output', 'ico_trend.csv'))
