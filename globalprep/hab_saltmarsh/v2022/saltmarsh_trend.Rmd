...
foreach (i = files) %dopar% { 
  ...
  if(!file.exists(file_name)){
    ...
    cat(paste0("Opening: ", files[i], "\n"))
    
    img <- terra::rast(paste0(path, i))
    
    cat("Filtering for Tidal Marsh...\n")
    ...
    cat(paste0("Saving file as: /int/type/", fn, "\n"))
  } else {
...
# tictoc::tic()
# 
# paths <- list.files(paste0(dir_data, "/raw"), pattern = "Type", full.names = TRUE)
# files <- list.files(paste0(dir_data, "/raw"), pattern = "Type")
# 
...
file <- list.files(paste0(dir_data, "/raw"), pattern = "gainYear-0000065536-0001114112", full.names = TRUE)
...
```{r eval = FALSE}
tictoc::tic()
paths <- list.files(paste0(dir_data, "/raw"), pattern = "Year", full.names = TRUE)
files <- list.files(paste0(dir_data, "/raw"), pattern = "Year")
years <- c(1, 4, 7, 10, 13, 16, 19)
...
for (i in 1:length(paths)) {
  for (yr in years){
    ...
    print("Filtering for Tidal Marsh...")
    ...
    terra::subst(
      ...
      from = yr,
      to   = 1,
      filename = paste0(dir_data, "/int/year/", fn)
    )
    
    cat(paste0("Saving file as: /int/year/", fn, "\n"))
  }
}
...
```{r eval = FALSE}
scale_factor = 30
img <- list.files(paste0(dir_data, "/raw"), pattern = "gain-", full.names = TRUE)

img_rast_agg <- lapply(sprc(lapply(img, rast)), terra::aggregate, fact = scale_factor, fun = sum)

for (i in 1:length(img_rast_agg)) {
  fn = paste0("gain-upscaled_", scale_factor, "_", i, ".tif")
  terra::writeRaster(img_rast_agg[[i]], paste0(dir_data, "/int/", fn))
  print(i)
}
# for (ras in 1:length(img_rast_agg)) {
#   terra::plot(img_rast_agg[[ras]])
# }
```
