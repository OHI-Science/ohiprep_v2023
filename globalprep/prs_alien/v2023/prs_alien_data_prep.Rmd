...
plot_diff <- 
  ggplot2::ggplot(
    compare, ggplot2::aes(x = `2023`, y = `2008`, text = rgn_name, label = rgn_id), color = "black") + # make sure to update version year here
...
prev_yr_data <- here::here('globalprep', 'prs_alien', 'v2022', 'output', 'prs_alien.csv') %>% # make sure to change version year to previous year
  readr::read_csv() %>%
  dplyr::left_join(rgns_eez)

compare_prev_yr <- new_data %>%
  dplyr::bind_rows(prev_yr_data) %>% 
  tidyr::pivot_wider(names_from = year, values_from = pressure_score)

plot_diff_prev_yr <- 
  ggplot2::ggplot(
    compare_prev_yr, ggplot2::aes(x = `2023`, y = `2022`, text = rgn_name, label = rgn_id), color = "black") + # make sure to update years here
  ggplot2::geom_jitter(width = 0.025, height = .025) +
  ggplot2::geom_abline() +
