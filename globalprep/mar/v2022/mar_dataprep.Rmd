...
Steps: 
 ...
 - Assign any Algae with the seaweeds score from SFW data (7.92) (link to the SFW Global Assessment for Seaweed available for download  [here](https://www.seafoodwatch.org/recommendation/seaweed/seaweed-31568)
...
mar_sw_sus <- mar_sw_sus %>%
  ...
  mutate(avg_rgn_fam_sust = mean(Sust, na.rm = TRUE), # produced NaN values (2022 secario year)
         avg_rgn_fam_esc = mean(esc_score, na.rm = TRUE)) %>% # produced NaN values (2022 secario year)
  ungroup() %>%
  group_by(georgn_id, family) %>%
  mutate(avg_gr_fam_sust = mean(Sust, na.rm = TRUE), # produced NaN values (2022 secario year)
         avg_gr_fam_esc = mean(esc_score, na.rm = TRUE)) %>% # produced NaN values (2022 secario year)
  ungroup() %>%
  group_by(family) %>%
  mutate(avg_fam_sust = mean(Sust, na.rm = TRUE), # produced NaN values (2022 secario year)
         avg_fam_esc = mean(esc_score, na.rm = TRUE)) %>% # produced NaN values (2022 secario year)
  ungroup() %>%
  group_by(Taxon_code) %>%
  mutate(avg_taxon_code_sust = mean(Sust, na.rm = TRUE), # produced NaN values (2022 secario year)
...
# delete:
# # code NA values as NaN for new columns
# avg_rgn_fam_sust
# avg_rgn_fam_esc
# avg_gr_fam_sust
# avg_gr_fam_esc
...
#nan_cols <- c("avg_rgn_fam_sust", "avg_rgn_fam_esc", "avg_gr_fam_sust", "avg_gr_fam_esc", "avg_fam_sust", "avg_fam_esc", "avg_taxon_code_sust", "avg_taxon_code_esc")

#nan_cols <- c(mar_sw_sus$avg_rgn_fam_sust, mar_sw_sus$avg_rgn_fam_esc, mar_sw_sus$avg_gr_fam_sust, mar_sw_sus$avg_gr_fam_esc, mar_sw_sus$avg_fam_sust, mar_sw_sus$avg_fam_esc, mar_sw_sus$avg_taxon_code_sust, mar_sw_sus$vg_taxon_code_esc)
#nan_cols
# for (col in length(nan_cols)){
#   column <- mar_sw_sus$col
#   column[is.nan(col)] <- NA
# }

# for (i in seq_along(nan_cols)){
#   col <- paste0(mar_sw_sus, "$", i)
#   col[is.nan(col)] <- NA
# }
...
mar_sw_sus_final = mar_sw_sus %>%
  ...
  mutate(Sust = ifelse(Taxon_code == "AL", 6.72, Sust)) %>% ## assign any algae with the SFW seaweed score (6.72)
