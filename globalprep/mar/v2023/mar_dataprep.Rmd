...
mar <- mar %>% 
  pivot_longer(cols = paste0("1950":last_data_year),
               names_to = "year",
               values_to = "value") %>% 
  fao_clean_data_new() 
...
mar <- mar %>%
  mutate(country = case_when(country=="Réunion"~"Reunion", 
                             ...
                             TRUE ~country)) %>%  # This one is hard to get right; v2020: last year it was "R\xe9union", but this year it was "Réunion"
...
data.frame(filter(mar_rgn, rgn_id==80) %>%
  filter(year==2021) %>%
  arrange(species))
```
...
mar_rgn_gf <- pivot_longer(mar_rgn_spread, cols = num_range("",paste0(1950:last_data_year)), names_to = "year", values_to = "value") %>% arrange(rgn_id, species, year, Taxon_code, family, fao, environment)
...
##7381 of these out of 33995 + 7381 cases had NA values converted to 0 -2023 assessment
...
mar_old <- read.csv("../v2022/output/MAR_FP_data.csv") %>% 
  filter(year == 2020) %>% 
