...
mar <- mar %>%
  mutate(country = case_when(country=="Réunion"~"Reunion", 
                            ...
                            country == "United Republic of Tanzania, Zanzibar" ~ "Tanzania",
                             ...
                             TRUE ~country)) %>% mar_split()  # function in mar_fxs.R
...
#this creates duplicate species which we will need to sum
#code is new and in progress
maric <- maric %>%
  group_by(fao, environment, species, year, Taxon_code, rgn_id, family) %>% 
  summarize(value = sum(value), species_code = max(species_code), gap_0_fill =first(gap_0_fill)) %>%
  ungroup()


#run again after cleaning to check it worked
sort(setdiff(sw_sus_rgn$species, maric$species)) # Seafood Watch species with no FAO data
...
# 16364 NA values out of 33543 total obs ~ 49% without scores (2023)
```
...
mar_old <- mar_old_data %>% 
  filter(rgn_id == 68, year == previous_latest_data_year) %>% 
  select(rgn_id, species, fao, old_value = value)
mar_new <- mar_new_data %>% 
  filter(rgn_id == 68, year == latest_data_year) %>% 
  select(rgn_id, species, fao, value)
