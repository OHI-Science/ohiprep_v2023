...
-   New year of FAO mariculture yield data for 2021.
-   updated the species_list.csv so that any of the duplicate species that were relevant and had been excluded are now included. Also updated this file with any new species names in the fao data. Also removed the alias column, as it was being inconsistently used, and instead grouped by family when removing species with less than four years of mariculture for a region. Reorganized code as needed to keep this from creating errors. 
-   updated code to programmatically fill in analysis and data years throughout script
- fixed an issue in the mar_split function (mar_fxs.R) that was duplicating Netherlands data by binding the same rows multiple times. 
-  Fixed a problem that was removing data due to duplicates in the fao column (FAO major fishing area (Name)) for some species/region/environment/year combinations in the FAO data. See issue 246 for more information. 

- updated gather to pivot_longer
...
**Reference**:\
<https://www.fao.org/fishery/en/statistics/software/fishstatj> Release date: March 2023 FAO Global aquaculture production Quantity (1950 - 2021) Global aquaculture production (Value) (1950-2021)

FAO metadata found [here](https://www.fao.org/fishery/en/collection/aquaculture?lang=en).

**Downloaded**: May 8th, 2023

Note that for future years it appears the data can be directly downloaded through the link included above without using the fishstatj software.

Instructions on downloading the data: Navigate to this link: https://www.fao.org/fishery/statistics-query/en/home. Click browse on Global aquaculture production. Click browse on Global aquaculture production Quantity (1950 - 2021). Go to the bottom and de-select all of the pre-selected years. Scroll to the query definition section and drag Country Name En, ASFIS species Name En, FAO major fishing area Name En, and Environment Name En, into the selected rows column. Click on show data and confirm that data is present for 1950- two years prior to current date. Click download and select yes to include Null Values. Upload the file to the FAO_mariculture folder on MAZU. 

**Description**: Quantity (tonnes) and Value (thousand US\$) of mariculture for each country, species, year.
...
latest_data_year <- current_year-2 #most up to date year in the data
...
# for scen_year 2023, Gage already downloaded mar production data to git_annex
#update the data file name to the current last year of data 
mar <- read.csv(file.path(dir_M, "git-annex/globalprep/_raw_data/FAO_mariculture", current_data_folder, "FAO_GlobalAquacultureProduction_Quantity_1950-2021.csv"), check.names=FALSE, stringsAsFactors=FALSE) ; head(mar)
```
...
mar <- mar %>%
  ...
  rename_at(vars(matches("\\]")), ~ str_remove(., "\\]")) %>% 
  filter(!str_detect(country, "Fishery and Aquaculture Statistics")) %>% 
  filter(!str_detect(country, "Totals - Tonnes")) #removing rows that are not actually part of the data
...
mar <- mar %>%
  mutate(country = case_when(country=="Réunion"~"Reunion", 
                             country == "Micronesia (Fed. States)" ~ "Micronesia", 
                            ...
                            country == "United Republic of Tanzania, Zanzibar" ~ "Tanzania",
                             ...
                             country == "Yugoslavia SFR" ~  "Montenegro",
                             TRUE ~country)) %>%
...
data.frame(filter(mar_rgn, rgn_id==202) %>%
  filter(year==2020) %>%
  arrange(species))
```
...
##7380 of these out of 34185 + 7380 cases had NA values converted to 0 -2023 assessment
...
# 16364 NA values out of 33732 total obs ~ 49% without scores (2023)
```
...
tmp_old <- #read old and new data for plots
mar_old_data <- read.csv(file.path("..", paste0( "v", last_year), "output/mar_sustainability.csv")) %>% 
  select(rgn_id, taxa_code, sust_old = sust_coeff)
...
## Compare genetic escapes pressure scores for Vietnam; no change in pressures between 2023 and 2021 assessment years.
old <- read.csv(file.path("..", paste0( "v", last_year), "output/GenEsc.csv")) %>%
  filter(rgn_id == 232) %>% 
  select(rgn_id, year, prs_score_old=pressure_score)
