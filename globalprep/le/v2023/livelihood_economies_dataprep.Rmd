...
available, the original data used in the livelihoods and goal in 2013 is
included. For more details on what data is available, please refer to
the spreadsheet linked in the livelihoods and economies issue.
...
released when updating this goal is continued, an updated version of the
data should be downloaded and used.

-   FAO Capture Production Quantity

    -   The [FAO capture
        production](https://www.fao.org/fishery/en/collection/capture?lang=en)
        data was downloaded via the fishstat query portal. This data is
        updated annually and can be downloaded using the following
        instructions:
...
-   FAO Aquaculture Production
    ...
    -   Data from the [Global Aquaculture Production](#0) dataset:
        Global Aquaculture production Value, is used for mariculture
        revenue.
...
### FAO capture production quantity and exvessel price data
...
match as many of these species to the IUCN red-list and query the IUCN
api for habitat information.
...
There are a few data options for revenue from tourism, see read me for discussion on options. This component will need to be added before proceeding with the livelihoods and economies update. 


```{r}
all_wttc_data <- read_excel("~/OHI_repositories/ohiprep_v2023/globalprep/tr/v2022/raw/unwto-all-data-download_0.xlsx", sheet = "Inbound Tourism-Expenditure")

# Set the 2nd row as column names
colnames(all_wttc_data ) <- unlist(all_wttc_data[2, ])

# Assign a name to the 5th column
new_name <- "expenditure_type"  # Replace with your desired name
colnames(all_wttc_data)[5] <- new_name
colnames(all_wttc_data)[4] <- "country"

# Remove columns with NA names
all_wttc_data <- all_wttc_data[, !is.na(colnames(all_wttc_data))]

all_wttc_data <- all_wttc_data %>% 
  fill(country, .direction = "down")

tour_expenditure <- all_wttc_data %>%
  filter(expenditure_type == "Tourism expenditure in the country") %>% 
  pivot_longer(cols = paste(1995:2020), names_to = "year", values_to = "value") %>% 
...
production value database. We will find then find the proportion of
revenue for each country that is associated with marine/brackish
species.
...
### Tourism Jobs Data Wrangling

The WTTC data was available with projections through 2030, and is the
same data source used in the livelihoods and economies goal. Real data
is available through x, with future years of data not available for free
from this source. However this is the most complete data set and would
still be a major update from the last calculation of the livelihoods and
economies goal, even if projections were not used. This data was already
cleaned in the v2022 tourism data prep. We will use this cleaned file
and modify slightly.
...
tourism_revenue_clean <- tourism_revenue %>% 
  mutate(unit = "person (1)",
         value = jobs_ct,
         sector = "tour",
         data_source = "UNWTO"
         ) %>% 
  select(rgn_id, rgn_name, year, value, unit, sector, data_source) %>% 
  filter(rgn_id <250 & year < 2021) 


```

```{r}
#next we will calculate the percent of the population which is coastal
#this was last calculated in 2021

mar_pop_25 <- read_csv("~/OHI_repositories/ohiprep_v2023/globalprep/mar_prs_population/v2021/output/mar_pop_25mi.csv")

#use the WDI package to get overall population
...
region_populations <- mar_pop_25 %>%
  left_join(rgns_eez, by = c("rgn_id")) %>% #add iso3c code
  left_join(population_total, by = c("year", "eez_iso3")) %>% #add population data
left_join(low_pop, by = "rgn_id") %>% #add low pop 
mutate(pop = case_when((is.na(population) & Inhabited == 0) ~ 0,
                       (is.na(population) & Inhabited == 1) ~ est_population, 
                       T ~ population)) %>% 
  mutate(cost_pop = popsum/pop) %>% 
  mutate(cost_pop = ifelse(cost_pop >1 ,1,cost_pop)) %>%
  select(rgn_id, year, rgn_name, popsum, pop, cost_pop)  
  

tourism_coastal <- tourism_revenue_clean %>% 
  left_join(region_populations) %>% 
  mutate(value_calc = value * cost_pop) %>%  #calculate the number of coastal jobs
select(rgn_id, year, value= value_calc, unit, sector, data_source)

write_csv(tourism_revenue_clean, here(data_path, "int/tour_jobs.csv"))
```
...
-   For countries with both marine and freshwater cetacean viewing, we
    adjusted by the proportion of marine revenue accordingly. Because
    some of the whale watching in O'Connor et al. focused on freshwater
    cetacean viewing, we categorized the target species listed for each
    country as freshwater or marine. For countries with both marine and
    freshwater species, we categorized the whale watching in those
    countries as either 50% or 90% marine, based on the number of marine
    versus freshwater target species and information provided in the
    report narrative. For Colombia and Indonesia, more detailed
    information in the report narrative allowed for a more precise
    determination of the percentage of marine-based whale watching. We
    applied these marine proportions to data on the number of whale
    watchers before converting these estimates into employment
    estimates.
...
World](http://www.nber.org/oww/) (OWW) database produced by Remco H.
Oostendorp and Richard B. Freeman in 2005. These data were drawn from
the International Labour Organization and subjected to a standardization
process (for more information, see [http://www.nber.org/oww/Technical
\_document_1983-2003_standardizationv3.pdf](http://www.nber.org/oww/Technical%20_document_1983-2003_standardizationv3.pdf)).
