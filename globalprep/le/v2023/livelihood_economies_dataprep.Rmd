...
It is used to read in and wrangle all available livelihoods and economies data as of September 2023. When newer updated data is not available, the original data used in the livelihoods and goal in 2013 is included. For more details on what data is available, please refer to the spreadsheet linked in the livelihoods and economies issue.
...
These data sources are updated regularly. If new years of data have been released when updating this goal is continued, an updated version of the data should be downloaded and used.

-   FAO Capture Production Quantity

    -   The [FAO capture production](https://www.fao.org/fishery/en/collection/capture?lang=en) data was downloaded via the fishstat query portal. This data is updated annually and can be downloaded using the following instructions:

        -   Navigate to the [online query portal](https://www.fao.org/fishery/statistics-query/en/capture/capture_quantity) for FAO Global Capture Production Quantity. Deselect all pre-selected years. Drag these fields into selected rows: ASFIS species name, FAO major fishing area name, ASFIS species ISSCAP group name En. ASFIS species Family scientific name, FAO major fishing areas, Inland/Marine areas Name en. Click on show data and confirm that data is present for 1950- two years prior to current year. Click download and select yes to include Null Values.

-   FAO commodities data

    -   Data from [FAO Global Aquatic Trade Statistics](https://www.fao.org/fishery/en/collection/global_commodity_prod): Aquatic trade - All partners aggregated (Value) is used to estimate revenue from aquarium fishing. This is the original data source used, but it has been updated since the livelihoods and economies goal was originally calculated.

    -   To download this data, navigate to the Aquatic trade - All partners aggregated (Value) [online query portal](https://www.fao.org/fishery/statistics-query/en/trade/trade_value). De-select all pre-selected years. Under query definition move "Reporting Name Country (En) ,"Commodity Name (En), and "Trade Flow Name (En)" to selected rows. Click on show data and confirm that data is present for 1976- two years prior to current year. Click download and select yes to include Null Values.

-   FAO Aquaculture Production

    -   Data from the [Global Aquaculture Production](#0) dataset: Global Aquaculture production Value, is used for mariculture revenue.

    -   To download this data, navigate to the Global aquaculture production Value [online query portal](https://www.fao.org/fishery/statistics-query/en/aquaculture/aquaculture_value). De-select all pre-selected years. Under query definition move "Country (En) ,"ASFIS species Name", "ASFIS species Family Scientific Name", "FAO Major Fishing area Name en" and "Environment name en" to selected rows. Click on show data and confirm that data is present for the entire time period available in the portal, the end date is typically two years prior to current year. Click download and select yes to include Null Values.

-   [OECD Sustainable Ocean Economy](https://stats.oecd.org/Index.aspx?QueryId=95228#) data

    -   This platform is being retired, this data set will likely be located in the [OECD Data Explorer](https://data-explorer.oecd.org/) in future years.
...
-   ex-vessel price database.
    ...
    -   Recommended citation (note that this is for the original database, we use a version of this that emlab updated): Melnychuk, M. C., Clavelle, T., Owashi, B., and Strauss, K. 2016. Reconstruction of global ex-vessel prices of fished species. - ICES Journal of Marine Science. <doi:10.1093/icesjms/fsw169>.

-   Fao 2019 Yearbook

    -   This data was published in pdf format [here](https://www.fao.org/fishery/static/Yearbook/YB2019_USBcard/navigation/index_intro_e.htm)

    -   Although this report is published annually, newer versions of the report do not separate fish, aquaculture and fishery processing employees in a way that is usable for our analysis.
