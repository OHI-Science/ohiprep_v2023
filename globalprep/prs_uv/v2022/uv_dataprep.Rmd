...
## Replacing this with librarian shelf which will search bioconductor 
...
source(paste0('http://ohi-science.org/ohiprep_v', current_yr, '/workflow/R/common.R'))

## File paths
raw_data_dir <- file.path(dir_M, "git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV", data_yr)
int_sp_data <- file.path(dir_M, "git-annex/globalprep/prs_uv", data_yr, "int") # intermediate spatial data location
out_dir <- file.path(dir_M, "git-annex/globalprep/prs_uv", data_yr, "output")
...
land <-  filter(ocean_shp, rgn_typ %in% c("land", "land-disputed", "land-noeez")) %>% st_geometry()
...
foreach(i = 2005:(max(yrs) - 4)) %dopar% {
  ...
  if(file.exists(file.path("/home/shares/ohi/git-annex/globalprep/prs_uv", data_yr, "output", 
  ...
  mol1km_masked <- resc_files[str_sub(resc_files, -22, -19) == i] %>%
    ...
    mask(ocean, 
         filename = file.path("/home/shares/ohi/git-annex/globalprep/prs_uv", data_yr, "output", 
