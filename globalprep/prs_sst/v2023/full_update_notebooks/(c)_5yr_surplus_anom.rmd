...
writeRaster(ref, filename = file.path(dir_int, "85_89_ref_anom.tif"),
            overwrite=TRUE)
...
foreach(i = seq(1986, yrs[length(yrs)-4])) %do% {
  ...
  ref <- rast(file.path(dir_int, "85_89_ref_anom.tif"))
  
  # Load OHI five year anomaly period raster stack
  five_year_rast <- rast(c(anom_files[substr(anom_files, 81, 84) %in% years]))
