...
# assign names to be the cell terrain type (eez, land, etc.), and the OHI region ID to each value in the vector so we can then convert to a tidy dataframe
names(extracted_unweighted) <- paste(ohi_regions_wgs$rgn_type, ohi_regions_wgs$rgn_id, sep = " ")

# create a df of all these cell values and their labels for terrain type and region id as their own columns
fishing_regional_no_ant <- plyr::ldply(extracted, rbind) %>%
  # rename the dataframe's default variable names for clarity
  rename(fishing_effort = "1",
         "rgn_type rgn_id" = ".id") %>% 
  # split the conjoined columns for region id and land type into individual columns
...
# check which region id's are present in this df
unique(fishing_all$rgn_id)
...
length(unique(data_density$rgn_id)) # 220
...
#plot(aruba_poly$geometry, col = "grey96", axes = FALSE, main = "Aruba Trawling & Dredging Fishing Effort 2014", legend = FALSE)
#plot(fishing_effort_2014, axes = FALSE, col = "red", add = TRUE)
# no points seen...why???
# try 2015
#fishing_effort_2015 <- terra::rast(paste0(dir_M, "/git-annex/globalprep/hab_prs_hd_subtidal_soft_bottom/v2022/final_rasters/dredging_trawling_combined_2015.tif"))
#plot(aruba_poly$geometry, col = "grey96", axes = FALSE, main = "Aruba Trawling & Dredging Fishing Effort 2014", legend = FALSE)
#plot(fishing_effort_2015, axes = FALSE, col = "red", add = TRUE)
# try another country with lots of fishing effort?
#china <- data_density %>% 
#  filter(rgn_id == 209)
#chn_poly <- ohi_regions_wgs %>% filter(rgn_id == 209)
#View(head(chn_poly))
# 2014 all regions 
#plot(chn_poly$geometry, col = "grey96", axes = FALSE, main = "China Trawling & Dredging Fishing Effort 2014", legend = FALSE)
#plot(fishing_effort_2014, axes = FALSE, col = "red", add = TRUE)
...
annual_data_density <- data_density %>% 
  ...
  mutate(total_effort_km2_millions = total_effort_km2/1000000)

annual_density_plot <- ggplot(annual_data_density, aes(year, total_effort_km2_millions)) +
  geom_point() +
  labs(title = "Demersal Fishing Effort per Km2 of Softbottom Habitat 2012-2020",
             x = "Year",
             y = "Fishing Effort\n(millions of hours per km2 soft bottom habitat)")
...
quant_99 <- data_density %>%
  ...
  mutate(effort_km2_millions = quantDensity/1000000) %>% 
  data.frame()

ggplot(quant_99, aes(year, effort_km2_millions)) + 
  geom_point() +
  labs(title = "Demersal Fishing Effort per Km2 of Softbottom Habitat 2012-2020: Annual 99th Quantile",
       x = "Year",
       y = "Fishing Effort\n(millions of hours per km2 soft bottom habitat)")
