...
# remove certain regions' eez's from list because there is so much fishing effort over the entire time period that the API cannot handle it & crashes
...
# 
#     for(j in eez_codes) {
#       print(paste0("Checking if file exists for ", i, " EEZ code ", eez_codes))
#   filename <- paste0(dir_M, "/git-annex/globalprep/_raw_data/global_fishing_watch/d2022/annual_mapping_api/", i, "_", j, "_effort.csv")
# 
...
### Loop through apparent fishing effort separately for certain country EEZ code combinations due to the large quantity of data over all years 
...
# the second eez code for china should be fine to process over all years
...
```{r}
#BQ_code_eez <- get_region_id(region_name = "BQ", region_source = 'eez', key = key)
```
...
```{r}
## CW
# cw_code_eez <- get_region_id(region_name = "CW", region_source = 'eez', key = key)
# 
# # will be able to process all codes for CW besides this one, so remove it for now, loop over the rest, then do that one last with years subset into groups
# cw_code_eez_subset <- cw_code_eez %>% 
#   filter(id != "22756")
# 
# cw_eez_codes <- cw_code_eez_subset$id
# 
# for (c in cw_eez_codes) {
#   
#   cw <- gfwr::get_raster(spatial_resolution = 'high',
#                                         temporal_resolution = 'yearly',
#                                         group_by = 'flagAndGearType',
#                                         date_range = '2012-01-01,2020-12-31',
#                                         region = c,
...
#              geartype = Geartype) %>%
#       # keep track of the administrative country for each EEZ, even after we combine all data into one dataframe: 
#       mutate(eez_admin_rgn = "CW") %>% 
#       select(year, apparent_fishing_hours, y, x, eez_admin_rgn, geartype) # note: we do not care about which region did the actual fishing, just about which country controls that EEZ, which is why we do not maintain the fishing region in the data moving forward
#     
#     # specify column types before saving the csv so we can correctly concatenate the rows later
#     cw$year <- as.numeric(cw$year)
#     cw$apparent_fishing_hours <- as.numeric(cw$apparent_fishing_hours)
#     cw$y <- as.numeric(cw$y)
#     cw$x <- as.numeric(cw$x)
#     cw$eez_admin_rgn <- as.character(cw$eez_admin_rgn)
#     cw$geartype <- as.character(cw$geartype)
#     
#     write_csv(cw, paste0(dir_M, "/git-annex/globalprep/_raw_data/global_fishing_watch/d2022/annual_mapping_api/CW_", c, "_effort.csv"))
#   
# }
# 
# 
# 
# # now process eez code 22756 split into 2 chunks of years to not overwhelm the API
# cw_code_eez <- get_region_id(region_name = "CW", region_source = 'eez', key = key)
# # the index for code 22756 is ___
# 
# cw_2012_2015 <- gfwr::get_raster(spatial_resolution = 'high',
#                              temporal_resolution = 'yearly',
#                              group_by = 'flagAndGearType',
#                              date_range = '2012-01-01,2015-12-31',
#                              region = cw_code_eez$id[],
#                              region_source = 'eez',
#                              key = key) %>%
# # rename columns for clarity:
#       rename(year = "Time Range",
...
#     
#     # specify column types before saving the csv so we can correctly concatenate the rows later
#     cw_2012_2015$year <- as.numeric(cw_eez_22756_2016_2020$year)
#     cw_2012_2015$apparent_fishing_hours <- as.numeric(cw_2012_2015$apparent_fishing_hours)
#     cw_2012_2015$y <- as.numeric(cw_2012_2015$y)
#     cw_2012_2015$x <- as.numeric(cw_2012_2015$x)
#     cw_2012_2015$eez_admin_rgn <- as.character(cw_2012_2015$eez_admin_rgn)
#     cw_2012_2015$geartype <- as.character(cw_2012_2015$geartype)
# 
# 
# cw_2016_2020 <- gfwr::get_raster(spatial_resolution = 'high',
#                              temporal_resolution = 'yearly',
#                              group_by = 'flagAndGearType',
...
#     
#     # specify column types before saving the csv so we can correctly concatenate the rows later
#     cw_2016_2020$year <- as.numeric(cw_2016_2020$year)
#     cw_2016_2020$apparent_fishing_hours <- as.numeric(cw_2016_2020$apparent_fishing_hours)
#     cw_2016_2020$y <- as.numeric(cw_2016_2020$y)
#     cw_2016_2020$x <- as.numeric(cw_2016_2020$x)
#     cw_2016_2020$eez_admin_rgn <- as.character(cw_2016_2020$eez_admin_rgn)
#     cw_2016_2020$geartype <- as.character(cw_2016_2020$geartype)
# 
# cw_all_years <- bind_rows(cw_2012_2015, cw_2016_2020)
# 
# write_csv(cw_all_years, path = paste0(dir_M, "/git-annex/globalprep/_raw_data/global_fishing_watch/d2022/annual_mapping_api/CW_22756_effort.csv"))
...
# 2022 assessment: __ files were removed because had 0 rows, indicating no EEZ codes in the GFW database for that region, or no fishing activity at all within those countries' EEZs in 2012-2020 (from GFW AIS detections)
```
...
fish_effort_trawl_annual <- fish_effort_trawl %>% 
  ...
  summarize(total_cw_eez1_2016_2020 = sum(apparent_cw_eez1_2016_2020, na.rm = TRUE)) 
...
# Read in OHI regions polygons - several polygons (rows) for each region becuase one polygon exists for each region & land type combo (eez, land)
...
ohi_regions_wgs <-  st_transform(x = ohi_regions, crs = "EPSG:4326") # class = sf dataframe

for (i in years_all) {
  print(paste("Processing ", i, " fishing effort."))
  
  # read in the annual fishing raster for all regions the represetns corrected trawling plus dredging data 
...
# save this as the final data perhaps?
...
catch_2016 <- read.csv(here("globalprep/hab_prs_hd_subtidal_soft_bottom/v2022/int/fishing_annual_csv/fishing_effort_2012")) %>%
