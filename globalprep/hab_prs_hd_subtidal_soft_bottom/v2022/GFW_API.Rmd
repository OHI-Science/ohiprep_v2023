...
## Combine country csv's into one dataframe in order to group by latitude, longitude, and year
...
# check length again to see how many files we end with
  ...
  print(paste0("Ending with ", num_files_end, " files."))
```

```{r}
usa <- read.csv(paste0(dir_M, "/git-annex/globalprep/_raw_data/global_fishing_watch/d2022/annual_mapping_api/all_regions/USA_effort_2012_2020.csv"))
chn <- read.csv(paste0(dir_M, "/git-annex/globalprep/_raw_data/global_fishing_watch/d2022/annual_mapping_api/all_regions/CHN_effort_2012_2020.csv"))
```
...
unique(fish_effort_all$eez_admin_rgn)
```
## Examine China's trawling
...
# assign EPSG 4326 as hte CRS
proj4string(fish_effort) = CRS("+init=epsg:4326") # code from Gage's footprint project
...
# ensure years are factors because that is the class of the columns in fidsh_effort_annual:
...
```{r}
# First pull in EEZ shapefile to serve as a base layer for the annual maps using regions_shape() and filter for only eez's and land
regions_shape()
...
plot(eez, col = "grey96", axes = FALSE, main = "fishing effort 2015", legend = FALSE)		
plot(fish_effort_2015, axes = FALSE, col = "red", add = TRUE)
