...
data_density <- fishing_all %>%
  ...
  filter(rgn_id <= 250) %>%
  mutate(raw_density = effort_sum/km2)
...
# ref_point_90 <- data_density %>%
#   group_by(year) %>%
#   summarize(quantDensity = quantile(raw_density, c(0.90), na.rm = TRUE)) %>%
#   data.frame()

#write.csv(ref_point_90, here("globalprep/hab_prs_hd_subtidal_soft_bottom/v2022/int/reference_point_quant_95.csv"), row.names=FALSE)

# retrieve the median quantile from the annual quantiles:
#ref_point_90_median <- median(ref_point_90$quantDensity)
...
# assign each annual quantile value to that year
# quant_90_2012 <- ref_point_90$quantDensity[1]
# quant_90_2013 <- ref_point_90$quantDensity[2]
# quant_90_2014 <- ref_point_90$quantDensity[3]
# quant_90_2015 <- ref_point_90$quantDensity[4]
# quant_90_2016 <- ref_point_90$quantDensity[5]
# quant_90_2017 <- ref_point_90$quantDensity[6]
# quant_90_2018 <- ref_point_90$quantDensity[7]
# quant_90_2019 <- ref_point_90$quantDensity[8]
# quant_90_2020 <- ref_point_90$quantDensity[9]
# 
# # Rescale the density:
# data_density_2012 <- data_density %>%
#   filter(year == 2012) %>% 
#   mutate(density_rescaled_median = raw_density/quant_90_2012) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density_2013 <- data_density %>%
#   filter(year == 2013) %>%
#   mutate(density_rescaled_median = raw_density/quant_90_2013) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density_2014 <- data_density %>%
#   filter(year == 2014) %>%
#   mutate(density_rescaled_median = raw_density/quant_90_2014) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density_2015 <- data_density %>%
#   filter(year == 2015) %>%
#   mutate(density_rescaled_median = raw_density/quant_90_2015) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density_2016 <- data_density %>%
#   filter(year == 2016) %>%
#   mutate(density_rescaled_median = raw_density/quant_90_2016) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density_2017 <- data_density %>%
#   filter(year == 2017) %>%
#   mutate(density_rescaled_median = raw_density/quant_90_2017) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density_2018 <- data_density %>%
#   filter(year == 2018) %>%
#   mutate(density_rescaled_median = raw_density/quant_90_2018) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density_2019 <- data_density %>%
#   filter(year == 2019) %>%
#   mutate(density_rescaled_median = raw_density/quant_90_2019) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density_2020 <- data_density %>%
#   filter(year == 2020) %>%
#   mutate(density_rescaled_median = raw_density/quant_90_2020) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))
# 
# data_density <- rbind(data_density_2012,
#                       data_density_2013,
#                       data_density_2014,
#                       data_density_2015,
#                       data_density_2016,
...
# ref_point_85 <- data_density %>% 
#   group_by(year) %>%
#   summarize(quantDensity = quantile(raw_density, c(0.85), na.rm = TRUE)) %>%
#   data.frame()
# 
# write.csv(ref_point_85, here("globalprep/hab_prs_hd_subtidal_soft_bottom/v2022/int/reference_point_quant_85.csv"), row.names=FALSE)

## Reference point = 95th quantile across all years
#ref_point_95 <- read.csv(here("globalprep/hab_prs_hd_subtidal_soft_bottom/v2022/int/reference_point_quant_95.csv"))
#ref_point_85_median <- median(ref_point_85$quantDensity) # v2022 95th: 3.84
...
# lump all years together and get the quantile and divide by that value 
# try 90, 95, 99
# post histograms

#ref_point_95_all_years <- quantile(data_density$raw_density, c(0.95), na.rm = TRUE)

## Rescale the density:
# data_density <- data_density %>%
#   mutate(density_rescaled_median = raw_density/ref_point_85_all_years) %>%
#   mutate(density_rescaled_median_capped = ifelse(density_rescaled_median > 1,
#                                                  1,
#                                                  density_rescaled_median))

hist(data_density$density_rescaled_capped)

# Winsorize in new column: if the data is above the 95th quantile, replace it with the 95th quantile
# mutate(quantDensity = if_else(
#                                 raw_density > quantile(x = raw_density, probs = c(0.95), na.rm = TRUE),
#                                 quantile(x = raw_density, probs = c(0.95), na.rm = TRUE),
#                                 raw_density))

# plot just the 2022 assessment trend for fishing effort before rescaling
...
# save that winsorized data to the 2022 folder
#write.csv(annual_data_density_adjusted, file = here("globalprep/hab_prs_hd_subtidal_soft_bottom/v2022/int/reference_point_quant_95.csv"), row.names = FALSE)

# compare to the same years from the 2020 data (95th quantile, the lowest quantile used for that assessment year)
...
# ggplot() +
#   geom_line(data = assessment_2020, aes(x = year, y = quantDensity), col = "red") +
#   geom_text(aes(2013, 90, label = "2022 Assessment\nGFW data\nquantile = 0.95", vjust = 1), col = "blue") +
#   geom_line(data = assessment_2022, aes(x = year, y = quantDensity), col = "blue") +
#   geom_text(aes(2014, 50, label = "2020 Assessment\nWatson catch data\nquantile = 0.99", vjust = 1), col = "red") +
#   labs(x = "Year",
...
# instead of using the median of the winsorized data to rescale, use the formula (x-min(x))/(max(x)-min(x))
# data_rescale_formula <- data_density %>% 
#   mutate(density_rescaled_formula = (density - min(density, na.rm = T))/(max(density, na.rm = T) - min(density, na.rm = T))) %>% 
#   mutate(density_rescaled_formula_capped = ifelse(density_rescaled_formula > 1,
#                                                     1,
#                                                     density_rescaled_formula))
```
...
length(unique(health_pressure$rgn_id)) # 220
...
# Calculate and save health for softbottom habitat and subtidal habitat
