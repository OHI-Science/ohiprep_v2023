...
# nested for loop to iterate over countries and eez's:
...
```{r}
# test on subset of countries: choose large countries for which we know there is plenty of fishing data 
rgns_eez_subset <- rgns_eez %>%
  filter(eez_iso3 %in% c("USA", "CHN", "THA", "RUS"))
  
foreach(r = rgns_eez_subset$eez_iso3) %do% {
  # extract all eez code for that country
  code_eez <- get_region_id(region_name = r, region_source = 'eez', key = key)
  # iterate through all eez codes for that country to extract fishing hours
  ...
  foreach(e = code_eez$id, .combine = rbind) %do% {
    ...
    print(paste0("Extracted all apparent fishing hours for ", r, " EEZ ID ", e))
