---
title: "OHI 2021: combine_all_nutrients"
...
for(yr in years){
  ...
  n_leach_manure <- list.files(file.path(prep, "animal_farm_int/STEP7_total_N_leaching"), pattern = yr, full = TRUE)
  
  n_volt_manure <- list.files(file.path(prep, "animal_farm_int/STEP7_total_N_volatizing"), pattern = yr, full = TRUE)
  ...
  volt_crop <- list.files(file.path(prep, "crop_predatalayers"), pattern = crop_pat_volt, full = TRUE)
...
Calculate total nutrient application per year for manure and crops
```{r}
