...
# Summary
This document describes the steps for obtaining and prepping data used for the land based nutrient data layer for clean waters. In specific, this data prep focuses on prepping the N and P pourpoint shapefiles from crop fertilizer and manure excretion, which will be fed into the plume model. 
...
## create vector of pourpoints to exclude from 2005, 2006, and 2007, since these have already run (DELETE THIS LATER GAGE)

fils <- list.files("/home/sgclawson/plumes/output1_try1/", pattern="tif$", full.names = FALSE, recursive = TRUE)
...
if(str_detect(file, "2005|2006|2007|2008")){
  
  ws_pp_zonal_all_fix <- ws_pp_zonal_all_fix %>%
    filter(!(basin_id %in% c(included_2005)))
} else{
  ws_pp_zonal_all_fix <- ws_pp_zonal_all_fix
}
...
st_write(ws_pp_zonal_all_fix, file.path(prs_int_dir, "pourpoints", final_name), delete_dsn = TRUE) ## this is our final pourpoint data set we will plug into the plume model 
...
```{r, eval = FALSE}
## Write out an Australia shapefile for each year to use for testing... 
...
ws_pp_zonal_all_fix <- st_read(file.path(paste0(prs_int_dir, "/pourpoints/", file))) %>%
  mutate(basin_id = as.character(basin_id))
...
au_pp_all <- ws_pp_zonal_all_fix %>%
 ...
 # top_n(100, effluent)
  filter(effluent < 1) %>%
  slice_min(n = 200, effluent)

# mapview(au_pp_all)

st_write(au_pp_all, file.path(paste0(prs_int_dir, "/pourpoints_small/", "small_", file)), delete_dsn = TRUE)
}
...
```{r}
files_tif <- list.files(file.path("/home/shares/ohi/git-annex/globalprep/prs_land-based_nutrient/v2021/output/au_test_N_plume/"), full = TRUE)
files_tif
...
test <- st_read(files_shp[1])

sum_df <- data.frame(sum=NA, name = NA)

for(file in files_shp){

test_au <- st_read(file)

test_df <- data.frame(sum = sum(test_au$effluent), name = basename(file)) # 184230.8

sum_df <- rbind(test_df, sum_df)

}



sum_df_2 <- data.frame(sum = NA, name = NA)

for(file in files_tif){
 
  test_rast <- raster(file)

  test_df <- data.frame(sum = cellStats(test_rast, "sum", na.rm = TRUE), name = basename(file))

  sum_df_2 <- rbind(sum_df_2, test_df)
...
test_total <- cbind(sum_df %>% arrange(name) %>% rename("sum_shp" = "sum", "name_shp" = "name"), sum_df_2 %>% arrange(name)) %>%
  mutate(diff = sum_shp - sum) # missing ~15k for each... I believe that these are just inland pourpoints that aren't being picked up by the plume model
 

test_total <- test_total %>%
  mutate(diff = sum_shp - sum)
