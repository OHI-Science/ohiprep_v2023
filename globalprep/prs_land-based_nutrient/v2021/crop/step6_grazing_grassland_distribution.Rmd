...
source(here('globalprep/prs_land-based_nutrient/v2021/crop/R/common_spatial.R'))
...
fao_nutrient_grassland_N <- 
  vroom(here(crop_nutrient, "crop/int/fubc_2017_grassland_2.csv")) %>% 
...
sum(df_all_3_K2O$cell_application, na.rm = TRUE) # 13221098
sum(fao_nutrient_grassland_K2O$country_application, na.rm = TRUE) # 13221285

13221098 - 13221285 # slightly off, but nothing of concern
...
grassland_fertilizer_application <- function(i, nutrient, this_year) {
 ...
 select(x, y, cell_grazer_application) %>%
 rasterFromXYZ(crs = food_crs) %>%
 projectRaster(., correct_proj, method = 'ngb') %>% ## ngb ensures the sum stays the same... no interpolation
 writeRaster(.,
             file.path(prep,
...
# write for loop to loop through the scenarios; for next year, maybe try dopar for parallel processing to speed up. Takes around 45 mins as it is.
for(j in 2010:2019){
...
sum(fao_nutrient_grassland_K2O$country_application, na.rm = TRUE), na.rm = TRUE) # 77726830
