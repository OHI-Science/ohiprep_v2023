...
tr_jobs_pct_change <- read_csv(here(paste0("globalprep/tr/v", version_year,
  ...
  select(-c(pct_change_2020, pct_change_2021, country, rgn_name))
...
tr_jobs_pct_est <- tr_jobs_pct_tour %>% 
  ...
  mutate("Ep_gf_2021" = case_when(!is.na(Ep_2021) ~ Ep_2021,
                                  TRUE ~ Ep_gf_2020 * (1 + prop_change_2021)))

# create df that indicates which values were gapfilled
tr_jobs_pct_gf <- tr_jobs_pct_est %>% 
  mutate(gf_2020 = case_when(!is.na(Ep_gf_2020) ~ "gapfilled")) %>% 
  mutate(gf_2021 = case_when(is.na(Ep_2021) & !is.na(Ep_gf_2021) ~ "gapfilled")) %>% 
  select(rgn_id, gf_2020, gf_2021,
         "2019" = Ep_2019,
         "2020" = Ep_gf_2020,
         "2021" = Ep_gf_2021)

# because we used the final (gapfilled) output from the previous update to supplement the 2019 TTDI data we need to pull in the associated gf table to show which 2019 values were gapfilled originally
prev_version_year <- as.character(as.numeric(version_year) - 1)

# vector of rgn_ids that had values in the 2019 TTDI - used below to exclude them from being labeled gapfilled
ttdi_reported <- ttdi_emp_rgn$rgn_id

tr_jobs_pct_gf_2019 <- read_csv(here(paste0("globalprep/tr/v", prev_version_year, "/output/tr_jobs_pct_tourism_gf.csv"))) %>% 
  filter(year == 2019, 
         !rgn_id %in% ttdi_reported) %>% 
  rename("gf_2019" = "gapfilled") %>% 
  select(-year)

tr_jobs_pct_gf <- tr_jobs_pct_gf %>% 
  left_join(tr_jobs_pct_gf_2019, by = "rgn_id")

# separate gf and other data into separate dataframes, pivot both, and then rejoin
gf <- tr_jobs_pct_gf %>% 
  select(rgn_id, gf_2019, gf_2020, gf_2021) %>% 
  rename("2019" = gf_2019,
         "2020" = gf_2020,
         "2021" = gf_2021) %>% 
  pivot_longer(cols = c("2019","2020","2021"), names_to = "year", values_to = "gapfilled")

tr_jobs_pct_gf <- tr_jobs_pct_gf %>% 
  select(-c(gf_2019, gf_2020, gf_2021)) %>% 
  pivot_longer(cols = c("2019","2020","2021"), names_to = "year", values_to = "Ep") %>% 
  left_join(gf, by = c("rgn_id", "year")) %>% 
  mutate(method = case_when(!is.na(method) ~ method,
                            !is.na(gapfilled) ~ "WTTC Percent Change"))
...
# v2020 371 NAs
# v2022 21 NAs (new gapfilling method)
...
# v2022: Adding the low pop df takes the total number of NAs in the data set from 21 to 60
