...
### Look at changes in values and outliers

From the plot created below we can see that the majority of regions saw decreased values between 2019 and 2021 - this is to be expected as the pandemic was catastrophic for many tourism industries. There are however several regions which appear to have increased

```{r}

tr_jobs_pct_2019 <- tr_jobs_pct_tourism %>% 
  filter(year == 2019) %>%
  select(-year) %>% 
  rename("Ep_2019" = "Ep")

compare <- tr_jobs_pct_tourism %>% 
  filter(year == 2021) %>% 
  select(-year) %>% 
  rename("Ep_2021" = "Ep") %>% 
  left_join(tr_jobs_pct_2019)

plot(compare$Ep_2019, compare$Ep_2021)
abline(0, 1)

# look at which regions saw increases
compare <- compare %>% 
  mutate(diff = Ep_2021 - Ep_2019)

rgns <- rgn_master %>% 
  select(rgn_id = rgn_id_2013, rgn_name = rgn_nam_2013)

lg_incrs_rgns <- compare %>% 
  filter(diff > 0.015) %>%  # greater than 1.5% increase
  left_join(rgns)

# regions with increases are Thailand, Netherlands, Georgia, Montenegro, and Malaysia - should look into why they increased
