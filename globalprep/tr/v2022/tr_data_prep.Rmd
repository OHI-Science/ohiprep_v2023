...
* Percent change in "Total contribution of Travel & Tourism to Employment", used to gapfill proportion of workforce directly employed in tourism: scraped from PDFs on the [WTTC Economic Impact page](https://wttc.org/Research/Economic-Impact)
...
These data are cleaned and formatted below. The TTDI employment data are not as comprehensive as the data that were retrieved directly from the WTTC, so missing 2019 values are supplemented with output from the previous update. Gapfilling is done first using percent change in "Total contribution of Travel & Tourism to Employment" values from the [WTTC individual country reports](https://wttc.org/Research/Economic-Impact) available as PDFs, and then done using UN georegion means.
...
# making the df tidy gets a little messy here because values are described both column-wise and row-wise. The loop below splits the df every two rows which isolates each different year and its respective job percent values — each df is then pivoted, and then finally all are bound back into one

# even though we won't actually use the 2018 values from the TTDI, this script will tidy values for all years included in the dataset so that it remains usable if we continue using the TTDI in the future

ttdi_emp <- ttdi_raw %>%
  filter(Title == "T&T industry Share of Employment, % of total employment",
         Attribute %in% c("Value", "Date description")) %>% 
  select(Attribute, Albania:Zambia)

# pull reported data years from df
years <- ttdi_emp %>% 
  filter(Attribute == "Date description") %>% 
  pull() %>% 
  unique()
...
for (i in seq_along(years)) {
  ...
  df_name <- paste0("tr_jobs_pct_tour_", i)
  row <- (i * 2) - 1
  
  # assign the output of the pipe sequence below that pivots and adds respective year column to the object `df_name`
  assign(df_name,
         ttdi_emp[row, ] %>% 
           select(-Attribute) %>% 
           pivot_longer(cols = everything(), names_to = "country", values_to = "jobs_pct") %>% 
           mutate(year = years[i]))
  
  df_list[[df_name]] <- get(df_name)
  
  # delete temporary split dfs
  rm(list = ls(pattern = "tr_jobs_pct_tour_\\d"))
}

ttdi_emp <- bind_rows(df_list) %>% 
  mutate(jobs_pct = as.numeric(jobs_pct),
...
# add region names and change percent to proportion
...
ttdi_reported <- ttdi_emp_rgn$rgn_id
...
tr_jobs_pct_tour <- tr_jobs_pct_tour %>% 
  full_join(tr_jobs_pct_v2021, by = c("rgn_id", "year"))
...
tr_jobs_pct_tour <- tr_jobs_pct_tour %>% 
  mutate(Ep = case_when(is.na(Ep) & !is.na(Ep_prev) ~ Ep_prev,
                        TRUE ~ Ep)) %>% 
  select(-Ep_prev)
...
# calculate 2020 Ep estimates using 2019 values and proportion change, and then repeat for missing 2021 values using the 2020 estimates
tr_jobs_pct_est <- tr_jobs_pct_tour %>% 
  left_join(tr_jobs_pct_change) %>% 
  pivot_wider(names_from = year, values_from = Ep) %>% 
  rename("Ep_2019" = "2019", "Ep_2020" = "2020") %>% 
  mutate("Ep_gf_2020" = case_when(!is.na(Ep_2020) ~ Ep_2020,
                                  TRUE ~ Ep_2019 * (1 + prop_change_2020))) %>% 
    mutate("Ep_gf_2021" = Ep_gf_2020 * (1 + prop_change_2021))

# create df that indicates which values were gapfilled
tr_jobs_pct_gf <- tr_jobs_pct_est %>% 
  mutate(gf_2020 = case_when(is.na(Ep_2020) & !is.na(Ep_gf_2020) ~ "gapfilled")) %>% 
  mutate(gf_2021 = case_when(!is.na(Ep_gf_2021) ~ "gapfilled")) %>% 
  select(rgn_id, gf_2020, gf_2021,
         "2019" = Ep_2019,
         "2020" = Ep_gf_2020,
         "2021" = Ep_gf_2021)


# separate gf and other data into separate dataframes, pivot both, and then rejoin
gf <- tr_jobs_pct_gf %>% 
  select(rgn_id, gf_2020, gf_2021) %>% 
  rename("2020" = gf_2020,
         "2021" = gf_2021) %>% 
  pivot_longer(cols = c("2020","2021"), names_to = "year", values_to = "gapfilled")


tr_jobs_pct_gf <- tr_jobs_pct_gf %>% 
  select(-c("2019", gf_2020, gf_2021)) %>% 
  pivot_longer(cols = c("2020", "2021"), names_to = "year", values_to = "Ep") %>% 
  left_join(gf) %>% 
  mutate(method = case_when(!is.na(gapfilled) ~ "WTTC Percent Change"))

```

### Assessing gapfill method efficacy

```{r}
# # create df with entirely gapfilled values for 2021
# tr_jobs_2021_test <- tr_jobs_pct_tour %>% 
#   left_join(tr_jobs_pct_change) %>% 
#   pivot_wider(names_from = year, values_from = Ep) %>% 
#   rename("Ep_2019" = "2019", "Ep_2020" = "2020", "Ep_2021" = "2021") %>% 
#   mutate("Ep_gf_2020" = Ep_2019 * (1 + prop_change_2020)) %>% 
#   mutate("Ep_gf_2021" = Ep_gf_2020 * (1 + prop_change_2021))
# 
# # see how they compare with the reported values
# ggplot(tr_jobs_2021_test, aes(x = Ep_gf_2021, y = Ep_2021)) +
#   geom_point() +
#   geom_abline() +
#   labs(x = "Gapfilled", y = "Reported", title = "Gapfilled vs reported values for 2021 proportion of\n
#        direct tourism employment to total employment") +
#   scale_x_continuous(limits = c(0, 0.15))

```

### Gapfilling (pt. 2) using UN georegions
...
tr_jobs_pct_gf <- tr_jobs_pct_gf %>%
  ...
  mutate(gapfilled = case_when(!is.na(gapfilled) ~ "gapfilled",
                               is.na(Ep) & !is.na(Ep_all) ~ "gapfilled")) %>%
  mutate(method = case_when(!is.na(method) ~ method,
                            is.na(Ep) & !is.na(Ep_pred_r2) ~ "UN georegion (r2)",
...
# v2020 371 NAs
# v2022 14 NAs (new gapfilling method)
...
# check to make sure values aren't too different between this year's assessment and the one prior - we expect some difference because of the switch to employment data from the TTDI, but because their original source is the WTTC it should be quite similar
...
# the apparent outlier is the Philippines - this could potentially be related to reporting before and after the very start of the pandemic in late 2019, however nothing is specifically mentioned about that value in the TTDI dataset
