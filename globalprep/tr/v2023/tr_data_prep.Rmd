...
* Ap = Proportion of arrivals to area of coastline (formerly Ep when it represented employment)
* Sr = (S-1)/5; Sustainability of tourism


## The following data are used:

* Numbers of tourist arrivals: obtained through the [UNWTO](https://www.unwto.org/tourism-statistics/key-tourism-statistics) (in the form of thousands of people). Range: 1995-2021
* Area of coastline, used to calculate proportion of arrivals to area of coastline: calculated in the [LSP goal](https://github.com/OHI-Science/ohiprep_v2023/tree/gh-pages/globalprep/lsp/v2023). Range: Static
...
* Proportion of tourist arrivals to area of coastline - UNWTO data on thousands of tourist arrivals, reported until 2021 (downloaded [here](https://www.unwto.org/tourism-statistics/key-tourism-statistics) (dataset: "Total arrivals" under Inbound Tourism) on 8/10/2023, used in combination with area of coastline calculated in the [LSP goal](https://github.com/OHI-Science/ohiprep_v2023/tree/gh-pages/globalprep/lsp/v2023), update completed on 6/26/2023)
...
# Ap: Proportion of tourist arrivals to area of coastline

We use tourist arrival data from the [United Nations World Tourism Organization (UNWTO) ](https://www.unwto.org/). Up until the current assessment, we accessed data from the [WTTC](http://www.wttc.org/), but this is no longer a viable option. This is in the form of thousands of people arriving, which we divide by area of coastline for each country. Missing values in this data are gapfilled by upfilling or downfilling years of data based on if the previous or later year has information; we were not able to further gapfill with other methods at this time.


### Calculating the proportion of tourist arrivals to area of coastline

The number of arrivals to area of coastline is used as a proxy for the value that people have for experiencing and enjoying coastal areas. We gather tourist arrivals and make it a proportion by dividing them by the area of coastline for the associated OHI region.

### Source cleaned arrivals and area of coastline data sources
...
source(here(paste0("globalprep/tr/v", version_year, "/R/process_UNWTO_arrivals.R"))) # outputs unwto_dupe_fix
# note: this script fills the total with values from Overnights or Same-day because it is not autofilled: Total originally is either a value on its own or the sum of Overnights and Same-day but must have both values, otherwise is NA

# source in prepared area of coastline data: uses current version year for these files by default
source(here(paste0("globalprep/tr/v", version_year, "/R/process_area_of_coastline.R"))) # outputs inland_offshore
```

### Divide tourist arrival count data by area of coastline

```{r}
# divide the number of tourist arrivals by area of coastline to get the proportion
tourism_props <- unwto_dupe_fix %>% 
  left_join(inland_offshore, by = c("rgn_id", "year")) %>%
  mutate(Ap = (tourism_arrivals_ct/total_inland_offshore_area))

# check out things so far
summary(tourism_props) # v2023: 435 NAs in Ap

# remove unnecessary columns after checking summary
tourism_props <- tourism_props %>%
  select(-tourism_arrivals_ct, -total_inland_offshore_area)
```

### Gapfill using previous/next years for same region
...
# prepare a "gapfilled" column to indicate "gapfilled" or NA
tourism_props_for_gf <- tourism_props %>%
    mutate(gapfilled = NA)
...
tourism_props_downup_gf <- tourism_props_for_gf %>%
  group_by(rgn_id) %>%
  arrange(rgn_id, year) %>%
  mutate(Ap = ifelse(year == 2021 & is.na(Ap), lag(Ap, n = 2), Ap)) %>%
  fill(Ap, .direction = "downup") %>%
  mutate(method = ifelse(is.na(method) & !is.na(Ap), "nearby year", method)) %>%
  mutate(gapfilled = ifelse(method == "nearby year", "gapfilled", gapfilled))

# check out things so far
summary(tourism_props_downup_gf) # v2023: 135 NAs in Ap
```

### Gapfilling using UN georegions (not used v2023)

To round out the gapfilling, we could use georegions. This has a very low correlation with the data, but it was the simplest viable option discovered. We decided ultimately just to leave values that would have been gapfilled by georegion as NAs since it did not seem to be reliable enough.
...
quantile_marker <- quantile(tourism_props_downup_gf$Ap, probs = 0.90, na.rm = TRUE) # 90th quantile = 1
tourism_props_downup_gf$Ap_rescaled <- tourism_props_downup_gf$Ap / quantile_marker # divide all values by this value
tourism_props_downup_gf$Ap_rescaled <- pmin(tourism_props_downup_gf$Ap_rescaled, 1) # any above 1 still rescale to 1

# exploratory checks
hist(tourism_props_downup_gf$Ap_rescaled, main = "Distribution of Rescaled Ap Values") # check out the distribution
perfect_scores <- tourism_props_downup_gf %>% filter(Ap_rescaled == 1) # get the times the Ap = 1
unique(perfect_scores$rgn_id) # which countries have at least one Ap value = 1?

# make rescaled column the actual value column
tourism_props_rescaled <- tourism_props_downup_gf %>%
  select(-Ap) %>%
...
tourism_props_rescaled <- tourism_props_rescaled %>% 
  ...
  mutate(gapfilled = ifelse(is.na(Ap), NA, gapfilled)) %>% 
  mutate(method = ifelse(is.na(Ap), NA, method))
...
plot(compare_common_data$Ap, compare_common_data$Ep,
