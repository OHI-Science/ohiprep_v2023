...
<br> The status represents the current status of the goal, the score is
an average of both the current status and the predicted future status.
Predicted future status incorporates resilience, pressures and trend.
<br>
...
scores %>% 
  filter(region_id == 1 & str_length(goal) < 3 &
      dimension %in% c("score")) %>% group_by(year) %>% 
...
this layer due to 2% historical global loss. I assume because the
resolution of available data does allow this to be calculate at the
regional level.** <br>
