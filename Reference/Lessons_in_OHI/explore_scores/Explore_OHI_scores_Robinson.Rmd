...
**Antarctica (region 213) has all NA values for scores which explains one of the extra regions. Region 0 does not have a name, this is global.**
...
**There appears to be both a region id and an admin region id. The admin region id is 163. However within that admin_region id, there are other region ids, presumably areas that are not geographically within the US but are territories of the us. These region ids are printed below. We don't ever really use the admin region id.**
...
**Ones with fewest have decent data.**
...
|            |                                                                                                                                                 |                                                                     |
|            |                                                                                                                                                 | predicted future status: sub dimensions-resilience, pressure, trend |
+------------+-------------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+
| region_id  | For the global OHI, each region is defined as the offshore EEZ area for all coastal countries and territories.                                  | 2020 total +(Anartica not calculated) Gloabl Region 0               |
+------------+-------------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+
| score      | Scores are calculated at the level of region and goal. Goal score represents an average of current and future status score.                     | 0-100                                                               |
+------------+-------------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+
...
artisinal <- scores_expanded %>% 
  filter(goal_name == "Artisanal Fishing Opportunity" & dimension == "score") %>% filter(year %in% c(2012,2022) & region_id != 0)
...
ggplot(artisinal, aes(x = region_id, y = score, col = score)) +
  ...
  facet_grid(~ year) + labs(title = "Artisinal Fishing Opportunity Scores for All Regions")


ggplot(artisinal, aes(x = region_id, y = score, col = score)) +
  geom_point() +
  facet_grid(~year) +
  geom_boxplot(alpha = 0) + labs(title = "Artisinal Fishing Opportunity Scores for All Regions")
...
scores_expanded %>% filter(goal_name == "Artisanal Fishing Opportunity" & dimension == "score") %>% 
  ...
  ggplot() + geom_histogram(aes(x = score), fill = "skyblue") + labs(title = "Distribution of OHI Scores", subtitle = " Artisinal Fishing Opportunity 2012-2023")
```

```{r}
scores_expanded %>% filter(goal_name == "Artisanal Fishing Opportunity" & dimension == "score") %>% 
  filter(year!= "2023") %>% #remove 2023 since this is a placeholder
  ggplot() + geom_density(aes(x = score), fill = "skyblue") + labs(title = "Density of OHI Scores", subtitle = " Artisinal Fishing Opportunity 2012-2023")
```
